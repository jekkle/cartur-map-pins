...
dotnet build (Join-Path $root "src\CarturMapPins.csproj") -c Release
...
Copy-Item (Join-Path $root "src\bin\Release\net472\CarturMapPins.dll") $plugins -Force

New-Item -ItemType Directory -Force -Path $dist | Out-Null
$zip = Join-Path $dist "Carturs_Map_Pins-$version.zip"
...
# file literally named "plugins\CarturMapPins.dll" instead of the folder.
...
$nexus = Join-Path $dist "Carturs_Map_Pins-$version-Nexus.zip"
if (Test-Path $nexus) { Remove-Item $nexus }
$archive = [System.IO.Compression.ZipFile]::Open($nexus, "Create")
try {
    [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
        $archive, (Join-Path $plugins "CarturMapPins.dll"), "BepInEx/plugins/CarturMapPins.dll") | Out-Null
}
finally { $archive.Dispose() }
